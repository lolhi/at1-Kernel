...
#define FEATURE_PN544_NOTI_WAKE_UP  //p50298 add
		...
		size_t count, loff_t *offset)
{
	...
	char tmp[MAX_BUFFER_SIZE];
	int ret;
#if defined(FEATURE_PN544_NOTI_WAKE_UP)  //p50298 add
	int fail_Cnt = 0;    
#endif
...
#if defined(FEATURE_PN544_NOTI_WAKE_UP)  //p50298 add
	if (ret == count)  //success
	{
		fail_Cnt = 0;
	}
	else  //fail
	{
		if(++fail_Cnt == 1)  //wake up notify
		{
			 pr_info("NFC Pn544 Waked Up By I2C! Addr : 0x%02x \n", pn544_dev->client->addr);
		}
	}
#endif
	if (ret != count) {
