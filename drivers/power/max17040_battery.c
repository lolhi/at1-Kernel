...
#define SKY_SOC_EMPTY	35 //original value :12
#define SKY_SOC_FULL	970 //original value : 984 
...
static u8 max17040_read_reg_bytes(u8 reg, char* buf, u8 num)
{
	u8 ret_s, ret_r, ret;
...
static void max17040_set_rcomp(void)
{
	int ret=0,i=0;	
	#ifdef FEATURE_AT1_PMIC_BATTERY
	ret=max17040_write_reg(MAX17040_RCOMP_MSB, 0x9b,0x00);
	#else
...
static void max17040_get_soc(void)
{
	...
	*/
	...
	else if(avalue >4300 && avalue <4435)
		soc=1;
	else 
		soc=0;
#endif
			...
			const struct i2c_device_id *id)
{
	...
	if((ret != -1) && (soc_buf[0] >= 0x75 && soc_buf[0] <= 0x77))
	{
