...
#ifdef FEATURE_AT1_USB_ANDROID_PTP
	{"printRoid",
	PRINT_TYPE_FLAG,
	0x10A9, 0xE007, "Pantech Config xx",  //<~~~~~~~~~~~ change....^^
	USB_CLASS_STILL_IMAGE, 1, 1},
#endif
...
#ifdef FEATURE_AT1_USB_ANDROID_PTP
int printRoid_mode_switch_cb(int enable)
{
	struct device_mode_change_dev *dev_mode_change = _device_mode_change_dev;
	int i;
	// jylee 121027 char *compare_name = "printRoid_mode";
	char *compare_name;  //jylee 121027
	char *pc_mode = "pc_mode"; //jylee 121027
	char *print_mode = "printRoid_mode"; //jylee 121027

	// jylee 121027 start
    if(enable)
    	compare_name = print_mode;
    else
        compare_name = pc_mode;
		// jylee 121027 end
...
static void android_enable_function_cb(struct android_usb_function **functions)
{
	...
	while ((f = *functions++)) {
		if((f->request_enabled != f->current_enabled) 
			...
			#ifdef FEATURE_AT1_USB_ANDROID_PTP
			else if(!strcmp(f->name, "printRoid")){
				if(f->request_enabled){
					
					(void)printRoid_mode_switch_cb(1); // jylee 121027
// jylee 121027					(void)printRoid_mode_switch_cb(0);
						
				}else{
				
				(void)printRoid_mode_switch_cb(0); // jylee 121027
// jylee 121027				type_switch_cb(DEFAULT_TYPE_FLAG);
			    }
			}
			#endif
...
#ifdef FEATURE_AT1_USB_ANDROID_PTP
	if(!strcmp(name, "printRoid")){
		while((f = *functions++)){
			if(!strcmp(f->name, "printRoid")){
				f->current_enabled = 1;
				list_add_tail(&f->enabled_list, &_android_dev->enabled_functions);
				return;
			}
...
#ifdef IS_AT1 /* jylee from  EF45  120814*/
static ssize_t usb_charging_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    } else if(!strcmp(buf, "mass_storage,adb")) { /* jylee add  only AT1_ICS 120814 */
        dev_mode_change->g_device_type = (MSC_TYPE_FLAG | ADB_TYPE_FLAG);
	}
