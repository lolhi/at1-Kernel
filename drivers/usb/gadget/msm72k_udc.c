...
static void update_cable_delay(void); 
...
static int usb_get_max_power(struct usb_info *ui)
{
	...
	#if !defined(IS_AT1) /* jylee 121009 */
	/* jylee This often occurs the problem that AT1 doesn't go to sleep after removed SDP cable */
...
static void update_cable_delay(void)
{
	...
	char conf_buf[128] = {'\0',};
	struct usb_info *ui = the_usb_info;
    ret = get_udc_state(conf_buf);
    chg_type = get_usb_charger_type();

	if ( (chg_type == 0) && strcmp(conf_buf,"CONNECTED")  )
