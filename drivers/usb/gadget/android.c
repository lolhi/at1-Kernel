...
#if defined(CONFIG_ANDROID_PANTECH_USB_ABNORMAL_CHARGER_INFO) /* jylee 121022 */
...
#ifdef FEATURE_AT1_USB_ANDROID_PTP
static int printroid_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	return printroid_setup();
}

static void printroid_function_cleanup(struct android_usb_function *f)
{
	printroid_cleanup();
}

static int printroid_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return printroid_bind_config(c, false);
}

static struct android_usb_function printroid_function = {
	.name		= "printRoid",
	.init		= printroid_function_init,
	.cleanup	= printroid_function_cleanup,
	.bind_config	= printroid_function_bind_config,
};
#endif /* FEATURE_AT1_USB_ANDROID_PTP */
