...
#if defined(AT1_WINDOW_TEST)
static u16 apds_threshold_high;
static u16 apds_threshold_low;
#define APDS9900_THRESHOLD_HIGH			35
#define APDS9900_THRESHOLD_LOW			25

//#define APDS9900_THRESHOLD_HIGH			apds_threshold_high//35//30
//#define APDS9900_THRESHOLD_LOW			apds_threshold_low//30//25
#else
...
/* -------------------------------------------------------------------- */
#if 1
void apds9900_set_near2far(void)	
{
	u16 set_num = apds_threshold_low<<4;
	apds9900_set_thrconfig(0x03FF, set_num);
}

void apds9900_set_far2near(void)
{
	u16 set_num = apds_threshold_high<<4;
	apds9900_set_thrconfig(set_num, 0x0000);
}
#endif

int apds9900_set_threshold(u16 threshold_num)
{
	if(threshold_num >= 35 && threshold_num <= 50)
	{
		apds_threshold_high = threshold_num;
		apds_threshold_low = (threshold_num -5);
		return 0;
	}
	else
	{
		apds_threshold_high = 35;
		apds_threshold_low = 30;
		return 1;
	}
}
...
/*	int thr; */  // forever Not  used .. remove..
	...
	if(apds9900data.active & APDS9900_TYPE_PROXIMITY) {
		...
		*/
		if(apds9900data.lastpsval.z >= apds_threshold_high/*APDS9900_THRESHOLD_HIGH*/) val->x = 0;
		else val->x = 5;
...
static int apds9900_set_irqdirection(void)
{
	...
	if(adc >= apds_threshold_high/*APDS9900_THRESHOLD_HIGH*/ ) apds9900_set_near2far();
...
{
	u8 oldenable = 0; // p50106 at1_ics 20121114 - switching sensor bug fixed
	int ret = 0;
	register_t reg = {
		.enable = 0,
		.atime = 0,
	...
	dbg("__________________ old : 0x%02X/ new : 0x%02X\n", oldenable, reg.enable);
	...
	if(active) {
		...
		//ret |= apds9900_set_reg(APDS9900_ENABLE_REG, reg.enable); // p50106 at1_ics 20121114 - switching sensor bug fixed
