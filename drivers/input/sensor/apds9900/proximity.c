...
#define AT1_FIX_TEST //20110906 bandit add for variable <prox threshold value> 
#if defined(AT1_FIX_TEST)
#define SENSOR_MIN_THRESHOLD	(35)
#define SENSOR_MAX_THRESHOLD	(50)
#endif
...
#if defined(AT1_FIX_TEST)  //hhs 20110831 test
static ssize_t sensor_threshold_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return 0;
}

static ssize_t sensor_threshold_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	int threshold_max = simple_strtoul(buf, NULL, 10);

	dbg_func_in();

	if (threshold_max < SENSOR_MIN_THRESHOLD) {
		threshold_max = SENSOR_MIN_THRESHOLD;
	}
	else if (threshold_max > SENSOR_MAX_THRESHOLD) {
		threshold_max = SENSOR_MIN_THRESHOLD;//SENSOR_MAX_THRESHOLD;
	}
    
	ret = apds9900_set_threshold((u16)threshold_max);
	printk(KERN_ERR "hhs sensor_threshold_store =%d <%d> \n",threshold_max,ret);
...
static DEVICE_ATTR(threshold, S_IRUGO|S_IWUSR|S_IWGRP,
        sensor_threshold_show, sensor_threshold_store);
#endif
