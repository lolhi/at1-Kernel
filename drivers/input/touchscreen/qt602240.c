...
#if 0//def SKY_PROCESS_CMD_KEY //p50253@yjs.misc
...
{
	uint8_t status = 0;
	uint16_t object_address_t7=0;
	#ifdef QT_MODEL_224
	uint16_t object_address_t9= 0;
	uint16_t object_address_t22= 0;
	uint16_t object_address_t28= 0;
	#endif
	#ifdef QT_MODEL_224_E
	uint16_t object_address_t48= 0;
	#endif
...
void set_charger_mode(unsigned long arg)
{
    ...
    if(tsp_power_status) {
        //printk("QT602240 set_charger_mode charger_mode = %d\n",charger_mode);
...
uint8_t calibrate_chip(void)
{
	...
	acquisition_config.atchfrccalthr = atchfrccalthr;
	acquisition_config.atchfrccalratio = atchfrccalratio; 
	#endif
...
void  get_message(struct work_struct * p)
{
	...
	if (object_type == TOUCH_MULTITOUCHSCREEN_T9)
	{
		...
		if(quantum_msg[1] == 0x10) {
			...
			if(!f_resume_cal) mod_timer(&tchautocal_timer, jiffies + msecs_to_jiffies(TOUCH_AUTO_CAL_TIME));				// after 6 sec, touch auto calibration will be disable.
	...
	if(cal_check_flag == 1 && touch_message_flag) { 
		if (mxt_time_point == 0) mxt_time_point = jiffies_to_msecs(jiffies);
...
{
	int rc; 
	struct regulator *vreg_touch, *vreg_touch1;
	...
	if (IS_ERR(vreg_touch))
	{
		...
		printk(KERN_ERR "%s: regulator get of vreg_touch failed (%d)\n",	__func__, rc);
	}
	...
	vreg_touch1 = regulator_get(NULL, "8058_l11");
	if (IS_ERR(vreg_touch1))
	{
		rc = PTR_ERR(vreg_touch1);
		printk(KERN_ERR "%s: regulator get of vreg_touch1 failed (%d)\n",	__func__, rc);
	}
	rc = regulator_set_voltage(vreg_touch1, 1900000, 1900000);
	if (rc) {
		printk(KERN_ERR "%s: vreg set level failed (%d)\n", __func__, rc);
		return rc;
	}
	rc = regulator_enable(vreg_touch1);
	if (rc) {
		printk(KERN_ERR "%s: vreg enable failed (%d)\n",__func__, rc);
		return rc;
	}
		
	// Return RST, CHG, I2C line
...
void off_hw_setting(void)
{
	...
	if (IS_ERR(vreg_touch1))
	{
 	...
 	rc = regulator_disable(vreg_touch1);
	if (rc) {
		printk(KERN_ERR "%s: vreg enable failed (%d)\n",__func__, rc);
		return;
	}
	regulator_put(vreg_touch1);
