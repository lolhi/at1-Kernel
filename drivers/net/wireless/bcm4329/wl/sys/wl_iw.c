...
wl_control_wl_start(struct net_device *dev)
{
	...
	WL_TRACE(("Enter %s\n", __FUNCTION__));
    /* 2011@osangsu :: AT1 add start */
    for(i=0; i<30; i++)  // wait for 1.5 sec 
    {       
       if((ret=get_lcd_status()) == FALSE)
       {
           printk("get_lcd_status OK : lcd on chkcnt: %d \n",i);
#ifdef CUSTOMER_HW_PT
	       if(i != 0) OSL_DELAY(100000); // wait for backlight on margin for 100 msec
#endif
           break;
       }
#ifdef CUSTOMER_HW_PT
	   OSL_DELAY(50000); // 50 msec
#endif
    }
    if(ret != FALSE)
    {
		WL_ERROR(("%s: get_lcd_status is TRUE \n", __FUNCTION__));
		return -1;
    }
    /* 2011@osangsu :: AT1 add end */
	if (!dev) {
		WL_ERROR(("%s: dev is null\n", __FUNCTION__));
		return -1;
	}
	...
	if  (g_scan_specified_ssid && (iscan_pass_abort == TRUE)) {
		...
		switch (status) {
			...
			case WL_SCAN_RESULTS_ABORTED:
				...
				if (g_scan_specified_ssid == 0)
                                {
//FEATURE_SKY_DS_WIFI_BLOCK_SCAN_ABORT
