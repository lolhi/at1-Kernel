...
#define DEFAULT_INPUT_MODE_CTL_REG_VAL  ( INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_3_DB | PGAINB_P_0_DB ) 
//#define DEFAULT_HP_MIXER_CTL_REG_VAL ( HPLMIX_NO_INPUT | HPRMIX_NO_INPUT )
#define DEFAULT_HP_MIXER_CTL_REG_VAL ( HPLMIX_INA2 | HPRMIX_INA1 )
...
#define DEFAULT_POWER_MANAGEMENT_CTL_REG_VAL  ( SHDN_EN  | LPMODE_DIS | SPKEN_DIS | HPLEN_DIS | HPREN_DIS | BYPEN_CLOSE )
...
static int delayDevice=0;  // 20120925,kyt : pop noise
...
int snd_extamp_write_all_reg(extamp_info_t tCurrentExtampInfo)
{
	...
	if(delayDevice>0) msleep(delayDevice);  // 20120925,kyt : pop noise
...
void snd_extamp_api_SetDevice(int on, uint32_t cad_device)
{
    ...
    if(CurrDeviceId == cad_device && CurrDevicePwrStatus == on)        return;
    ...
    if(on && curCallMode.mode == MODE_AUTOANSWER)
    {
		...
		case SND_DEVICE_HANDSET_RX:
			...
			delayDevice=0;  // 20120925,kyt : pop noise
		break;
		...
		if (on)
		{
			...
			tCurrentExtampInfo.hpl_ctl_reg_val           = ZCD_DIS | SLEW_EN | HPLM_UNMUTE | HPLVOL_M_4_DB;
			tCurrentExtampInfo.hpr_ctl_reg_val           = HPGAIN_P_0_DB | HPRM_UNMUTE | HPRVOL_M_4_DB;
			tCurrentExtampInfo.power_man_ctl_reg_val = SHDN_EN  | LPMODE_DIS | SPKEN_DIS | HPLEN_EN | HPREN_EN | BYPEN_OPEN;
			delayDevice=100; // 20120925,kyt : pop noise
		}
		else
		{
			tCurrentExtampInfo.inp_gain_ctl_reg_val  = INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_3_DB | PGAINB_P_0_DB;
			...
			tCurrentExtampInfo.power_man_ctl_reg_val = SHDN_EN  | LPMODE_DIS | SPKEN_DIS | HPLEN_DIS | HPREN_DIS | BYPEN_CLOSE;
			//tCurrentExtampInfo.power_man_ctl_reg_val = SHDN_DIS  | LPMODE_DIS | SPKEN_DIS | HPLEN_DIS | HPREN_DIS | BYPEN_CLOSE;
			delayDevice=0; // 20120925,kyt : pop noise
		}
		break;
		...
		if (on)
		{
			...
			tCurrentExtampInfo.inp_gain_ctl_reg_val   = INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_3_DB | PGAINB_P_0_DB;
			tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INA2 | HPRMIX_INA1;
			//tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INB2 | HPRMIX_INB2;  // speaker&headset used only spk path
			tCurrentExtampInfo.sp_mixer_ctl_reg_val = SPKMIX_INB2;
			tCurrentExtampInfo.hpl_ctl_reg_val = ZCD_DIS | SLEW_EN | HPLM_UNMUTE | HPLVOL_M_4_DB;
			tCurrentExtampInfo.hpr_ctl_reg_val = HPGAIN_P_0_DB | HPRM_UNMUTE | HPRVOL_M_4_DB;
			tCurrentExtampInfo.sp_ctl_reg_val = FFM_FF_MODE  | SPKM_UNMUTE | SPKVOL_M_2_DB;
			tCurrentExtampInfo.limiter_ctl_reg_val      = THDCLP_DIS |THDT1_1_DOT_4_SEC;
			//tCurrentExtampInfo.limiter_ctl_reg_val      = 0x10;
        ...
        if (on)
        {
            tCurrentExtampInfo.inp_gain_ctl_reg_val  = PGAINA_P_0_DB | PGAINB_P_0_DB;
            tCurrentExtampInfo.sp_ctl_reg_val = SPKM_MUTE ;
            tCurrentExtampInfo.hpl_ctl_reg_val  = tCurrentExtampInfo.hpr_ctl_reg_val = HPLM_MUTE;
            tCurrentExtampInfo.power_man_ctl_reg_val = tExtampInfo.power_man_ctl_reg_val;
            tCurrentExtampInfo.power_man_ctl_reg_val &= ~(BYPEN_CLOSE);
        }
        break;
...
static int snd_misc_device_init(void)
{
    ...
    if(result<0)    printk("snd_misc_device_init ret= %d ", result);
    return result;
}
static int snd_misc_device_deinit(void)
{
    ...
    if(result<0)    printk("snd_misc_device_deinit ret= %d ", result);
    return result;
}
static int ext_amp_open(struct inode *inode, struct file *file)
{
	//AMP_DBG_HIGH("ext_amp_open ");
	return 0;
}

static int ext_amp_release(struct inode *inode, struct file *file)
{
	//AMP_DBG_HIGH("ext_amp_release ");
	return 0;	
}
...
{
       uint32_t value;
	//AMP_DBG_MED("ext_amp_ioctl cmd:%d, arg:%d ", cmd, arg);
