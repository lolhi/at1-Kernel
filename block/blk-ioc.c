...
>>>>>>> 7e5a879... block, cfq: move io_cq exit/release to blk-ioc.c
{
	...
	ret = kmem_cache_alloc_node(iocontext_cachep, gfp_flags | __GFP_ZERO,
				    node);
	if (unlikely(!ret))
		return NULL;
	...
	INIT_RADIX_TREE(&ret->icq_tree, GFP_ATOMIC | __GFP_HIGH);
	INIT_HLIST_HEAD(&ret->icq_list);
	INIT_WORK(&ret->release_work, ioc_release_fn);
